...
class Tri(val p0: Vector3, val p1: Vector3, val p2: Vector3) {
    val plane = Plane()
    val vertecies = Array<Vector2>()

    init{
        plane.normal.set(p1.cpy().sub(p0).crs(p2.cpy().sub(p0)).nor())
        plane.constant = -(plane.normal.x*p0.x + plane.normal.y*p0.y + plane.normal.z*p0.z) //I sure hope this is the constant
        plane.calcUV()
        vertecies.add(plane.worldToPlane(p0))
        vertecies.add(plane.worldToPlane(p1))
        vertecies.add(plane.worldToPlane(p2))
    }
